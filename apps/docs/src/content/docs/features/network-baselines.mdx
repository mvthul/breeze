...
Baselines have a dedicated **Baselines** tab on the **Network Discovery** page. From there you can create a baseline for a site and subnet, enable or pause its scan schedule, trigger **Scan Now**, edit alert settings, and delete a baseline. The tab links out to the **Changes** tab to review the change events a baseline's scans have recorded. Use the **Known Guests** section (Settings > Partner > Regional) to manage the guest MAC allowlist described further down this page.

Everything the console does maps directly to the API calls documented below, so scripting the same workflow (e.g. for bulk provisioning) is fully supported.
